...
export function RideOptions() {
  return (
    <section id="ride-options" className="section-padding bg-surface-elevated overflow-hidden">
      <div className="container-wide">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-2 max-w-4xl mx-auto">
          {rides.map((ride, index) => (
            <FadeIn key={ride.name} direction="up" delay={index * 120} duration={650} scale>
              <article
                className={`card-premium relative overflow-hidden p-8 ${ride.accent} ${ride.dark ? 'border-ink/20' : ''}`}
              >
                {ride.featured && (
                  <span className="absolute right-6 top-6 rounded-full bg-brand px-3 py-1 text-xs font-semibold text-white shadow-sm">
                    Popular
                  </span>
                )}

                <div className="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-brand/10 transition-transform duration-300 group-hover:scale-110">
                  {ride.icon}
                </div>
